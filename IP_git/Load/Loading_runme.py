...
class LoadingRunMe(QMainWindow, Loading.Ui_MainWindow):
    ...
    def update_ui(self,count):
        if count == 1:
            self.label_1.setText(f"正在加载Basic components...")
            QApplication.processEvents()  # 刷新面板
        elif count == 2:
            self.label_1.setText(f"正在加载Elliptic Operator...")
            QApplication.processEvents()  # 刷新面板
        elif count == 3:
            self.label_1.setText(f"正在加载Generation Number...")
            QApplication.processEvents()  # 刷新面板
        elif count == 4:
            self.label_1.setText(f"正在加载Hash Operator...")
            QApplication.processEvents()  # 刷新面板
        elif count == 5:
            self.label_1.setText(f"正在加载Number Operation...")
            QApplication.processEvents()  # 刷新面板
        elif count == 6:
            self.label_1.setText(f"正在初始化...")
            QApplication.processEvents()  # 刷新面板
        elif count == 7:
            self.time_out()
        # elif count == 8:
        #     self.label_1.setText(f"正在加载Injection...")
        #     QApplication.processEvents()  # 刷新面板
        # elif count == 9:
        #     self.label_1.setText(f"正在加载Security Check...")
        #     QApplication.processEvents()  # 刷新面板
        # elif count == 10:
        #     self.label_1.setText(f"正在进入Layout...")
        #     QApplication.processEvents()  # 刷新面板
        # elif count == 11:
        #     self.label_1.setText(f"正在初始化...")
        #     QApplication.processEvents()  # 刷新面板
